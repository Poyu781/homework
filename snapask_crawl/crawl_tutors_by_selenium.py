...
def crawl_tutor_by_selenium(tutor_amount):
    driver = webdriver.Chrome(
        ChromeDriverManager(version="91.0.4472.101").install())
    # driver = webdriver.Remote(command_executor = "http://selenium:4444", desired_capabilities = DesiredCapabilities.FIREFOX)
    ...
    for i in range(1, click_times):
        element = WebDriverWait(driver, 10).until(
            EC.presence_of_element_located(
                (By.XPATH,
                 "//*[@id='app']/div/div/div/section[2]/button/div/div[1]")))
        element.click()

    store_data = []
    for i in range(1, tutor_amount + 1):
        # get all data inside the tutor div
        tutor_dom = driver.find_elements_by_xpath(
            f"//*[@id='app']/div/div/div/section[2]/div[2]/div[{i}]")
        data = tutor_dom[0].text.split("\n")
        # get img src and tutor_id by img tag
        image = driver.find_element_by_xpath(
            f"//*[@id='app']/div/div/div/section[2]/div[2]/div[{i}]/img")
        ...
        for index in range(len(data)):
            ...
            if index == 0:
                ...
                tutor_info_dict["answered_count"] = int(
